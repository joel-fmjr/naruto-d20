...
function mapDuration(raw = "") {
    ...
    if (/\/\s*level/i.test(s)) {
        ...
        if (gen) {
            ...
            return { units: unitMap[gen[2].toLowerCase()] ?? "round", value: gen[1] === "1" ? "@cl" : `${gen[1]}*@cl` };
        }
    }
...
let total = 0, populated = 0, skipped = 0, noAction = 0;
...
for (const filename of files) {
    ...
    if (DRY_RUN) {
        console.log(`[dry-run] ${doc.name} → actionType=${actionType}, damage=${action.damage?.parts?.map(p => p.formula + (p.types[0] ? "/"+p.types[0] : "")).join(",")||"none"}, save=${action.save?.type ?? "none"}, range=${action.range?.units ?? "none"}`);
    } else {
