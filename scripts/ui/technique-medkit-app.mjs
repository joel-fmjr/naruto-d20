...
const STATUS_META = {
    [STATUS.OUT_OF_DATE]: { label: "NarutoD20.Medkit.Status.OutOfDate", css: "out-of-date", icon: "fa-triangle-exclamation", sync: true },
    [STATUS.UP_TO_DATE]:  { label: "NarutoD20.Medkit.Status.UpToDate",  css: "up-to-date",  icon: "fa-circle-check",        sync: false },
    [STATUS.ORPHAN]:      { label: "NarutoD20.Medkit.Status.Orphan",    css: "orphan",      icon: "fa-link-slash",          sync: false },
};
...
export class TechniqueMedkitApp extends Application {
    ...
    activateListeners(html) {
        ...
        root.querySelector('[data-action="select-outdated"]')?.addEventListener("click", () => {
            root.querySelectorAll('input.medkit-pick').forEach((cb) => { cb.checked = true; });
        });

        root.querySelector('[data-action="sync"]')?.addEventListener("click", async (ev) => {
            ev.preventDefault();
            const ids = [...root.querySelectorAll('input.medkit-pick:checked')].map((cb) => cb.dataset.itemId);
            if (!ids.length) {
