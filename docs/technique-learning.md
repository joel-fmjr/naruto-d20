...
- The **learn check** is built from Character Level + the discipline's ability
  ...
  the comparison target to the **technique's** `learnDC`.
...
≥1 skill rank), and take-10/no-take-20 on the roll dialog.
...
- The **attempts budget** depends on actor state (ability mod + skill ranks), so
  ...
  ```js
  const abilityMod = actor.system.abilities?.[ability]?.mod ?? 0;   // discipline's ability
  const ranks      = actor.system.skills?.[skillKey]?.rank ?? 0;
  const maxAttempts = 1 + abilityMod + Math.floor(ranks / 2);
  ```
...
1. **Eligibility gate** (warn + abort on failure):
   ...
   - `< 1` rank in `DISCIPLINE_SKILL_MAP[discipline]` → warn.
...
2. **Failure Insight reset across techniques:** track the actor's
   ...
   (`flags["naruto-d20"].learning.currentTechniqueId`). If the player attempts a
   *different* technique, reset that technique's `failureInsight` to 0 and point
   the flag at the new one (per "lasts … until you attempt to learn a different
   technique").
3. **Build & roll** the discipline check the same way the Chakra-tab button does
   ...
   read the total (same result-capture approach used in `use-technique.mjs`).
...
2. **Failure Insight scope flag** — store the "currently training" technique id
   on an actor flag (planned) vs. infer it some other way.
